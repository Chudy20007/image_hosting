...
<div class='row rowPictures'>
    <div class='offset-3'>
        ...
        <div class='col-md-6 col-sm-12 picture text-center'>
                <h2>Profile</h2>
                <a class='profile-data' href="{{URL::asset('user/'.$pictures[0]->user->id)}}"><img class="img-thumbnail picture-icon" src="{{ URL::asset('css/img/avatars/'.$pictures[0]->user->id.".jpg ")}}"></a></br>
           <b> Name: </b> <a class='profile-data' href="{{URL::asset('user/'.$pictures[0]->user->id)}}">{{$pictures[0]->user->name}}</a> </br>@php $reg=explode(" ",($pictures[0]->user->created_at));@endphp
           <b>  Registered: </b>{{$reg[0]}}</br>
           <b>  Pictures count: </b> {{count($pictures)}} 
           <br/>
           @if  ($pictures[0]->id==Auth::id())
           <a href="{{URL::asset('/user/'.$pictures[0]->id.'/edit')}}"><button class="btn-info btn" id="buttonTestClick" value="" type="button">Edit profile</button></a>
            @endif
        </div>
        <div class='offset-3'>
            </div>
...
<div class='col-md-3 col-sm-3 picture'>
    ...
    <span class='image-comments'>Average rating: <b>{{$picture->average_rating()}} </b></span> </br>
            <a href="{{URL::asset('/pictures/'.$picture->id)}}">
                ...
                <span class='image-comments'>{{$picture->getCommentsCount()->count()}} comment(s)</span> </br> </br>             
            </a>
