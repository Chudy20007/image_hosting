...
<div class='col-md-3 col-sm-3 picture'>
    ...
    <div class="card-footer author">
        ...
        <div class='div-comments'>
            ...
            <span class='image-comments'>Visibility:
                <b>{{$picture->visibility}} </b>
            </span>
            </br>
            <span class='image-comments'>Active comments:
                <b>{{$picture->active_comments == 1 ? 'yes' : 'no'}} </b>
            </span>
            </br>
            <span class='image-comments'>Active ratings:
                <b>{{$picture->active_ratings == 1 ? 'yes' : 'no'}} </b>
            </span>
            </br>
            <br/>
            <div class='row'>
                @if (strpos($picture->uploadLink, $picture->id.'null') === false)
                <button class="btn-info btn link col-md-12" id="buttonTestClick" value="{{URL::asset('/show_pic/'.$picture->uploadLink)}}"
                    type="button">Copy link</button>
                @endif
            </div>
            <br/>
            <div class='row text-center'>
                <a class="col-md-12" href="{{URL::asset('/pictures/'.$picture->id.'/edit')}}">
                    <button class="btn-info btn col-md-12" value="" type="button">Edit picture</button>
                </a>
            </div>
            <br/>
            <div class='row text-center'>
                <a class="col-md-12" href="{{URL::asset('/pictures/'.$picture->id.'/destroy')}}">
                    <button class="btn-info btn col-md-12" value="" type="button">Delete picture</button>
                </a>
            </div>
            <br/>
            <div class='row text-center'>
                <a class="col-md-12" href="{{URL::asset('/pictures/'.$picture->id.'/add_visitors')}}">
