...
<div class='row rowPictures text-center'>
    ...
    <div class='row'>
        ...
        <div class='row'>
            <div class='col-md-12 col-sm-12'>
                    <a href="{{ URL::asset('pictures/'.$picture[0]->id)}}"><img class='img-thumbnail picture-show' src="{{ URL::asset($picture[0]->source)}}"></a>
            </div>
        </div>
        ...
        <div class="card-footer author">
            ...
            Added by:
            <b><a class='author-link' href="{{URL::asset('user/'.$picture[0]->user->id)}}">{{$picture[0]->user->name}}</a></b>

            <div class='div-comments'>
                    <?php $b=1; ?>
                    @if ($picture[0]->active_ratings)
                    
                    <div class='{{$picture[0]->id}}'>
                        @if (isset($picture[0]->user_rate))
                        @for ($i=0; $i<5; $i++)
                        
                       
                            @if ($i < $picture[0]->user_rate->rate)                   
                    ...
                    @endif
        <span class='image-comments'>Average rating: <b>{{$picture[0]->average_rating()}} </b></span> </br>
                <a href="#">
...
<div class='row rowPictures text-center'>
    <div class='col-md-12 col-sm-12 picture'>
        ...
        @endif
         @if ((count($picture)>0) || Auth::user()->isAdmin()) 
        <div class='row'>
            <div class='col-md-12 col-sm-12'>
                <blockquote>
                    <b> Comments</b>
        ...
        <div class="card-footer author">
                @php $i=0; @endphp
                @if ($picture[0]->active_comments == true)
            @foreach($picture[0]->comment as $pict)
...
@php $i++; @endphp
            ...
            @endphp
                @include('comments.create_fast_comment') @endif  @endif
        </div>
    </div>
</div>
@stop